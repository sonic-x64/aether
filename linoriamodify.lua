...
Library.NotificationAreaHolder = Library:Create('Frame', {
		BackgroundTransparency = 1;
		-- 1. Anchor to the bottom-center
		AnchorPoint = Vector2.new(0.5, 1);
		-- 2. Position at the middle X (0.5) and bottom Y (1), with a 40px offset from the absolute bottom
		Position = UDim2.new(0.5, 0, 1, -40);
		-- 3. Adjust size so it takes up the bottom half of the screen (plenty of room to stack)
	...
	Library:Create('UIListLayout', {
		...
		FillDirection = Enum.FillDirection.Vertical;
		SortOrder = Enum.SortOrder.LayoutOrder;
		-- 4. Force all notifications to stay perfectly centered horizontally
		HorizontalAlignment = Enum.HorizontalAlignment.Center;
		-- 5. Force the stack to start at the bottom and push upwards
...
function Library:Notify(Text, Time)
	...
	-- 1. Anchor to the center so it tweens outwards symmetrically
	NotifyOuter.AnchorPoint = Vector2.new(0.5, 0.5);
	
	-- 2. Position bottom-center of the screen (0.5 is middle X, 0.85 is bottom Y)
	NotifyOuter.Position = UDim2.new(0.5, 0, 0.85, 0); 
	
	-- Start with 0 width so it can expand
	...
	-- 3. Move the accent bar to the top instead of the left
	local TopColor = NotifyOuter.bar;
	TopColor.BackgroundColor3 = accent;
	-- Size is 100% width (Scale 1), and 2 pixels tall (Offset 2)
	TopColor.Size = udim2_new(1, 0, 0, 2); 
	TopColor.Position = udim2_new(0, 0, 0, 0); -- Positioned at the absolute top

	NotifyOuter.Parent = Library.NotificationArea;

	-- 4. Tween from center outwards (Left and Right simultaneously)
	local targetWidth = XSize + 8 + 4;
	pcall(NotifyOuter.TweenSize, NotifyOuter, UDim2.new(0, targetWidth, 0, YSize), 'Out', 'Quad', 0.4, true);
	
	task.spawn(function()
		task.wait(Time or 5);

		-- 5. Tween size back to the center (Width back to 0)
...
Size = UDim2.new(0, TabButtonWidth + 8 + 4, 1, -10);
    ...
    local AccentStroke = Library:Create('UIStroke', {
		Color = Color3.new(0, 0, 0); -- You can change this to match your UI's outline color
		Thickness = 1;
		Transparency = 1; -- Set to 1 so it remains hidden by default along with the AccentLine
		Parent = AccentLine;
	});
